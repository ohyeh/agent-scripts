#!/usr/bin/env bash
# Bill-of-lading validator (A2, warn-only): checks a delegated-agent prompt
# for the GOAL/ACCEPTANCE/REPORT sections required by
...
# This script only reports; the PreToolUse hook (.agents/hooks/bol-prompt-warn.sh)
# that wraps it decides never to block on the result.
set -u
