...
Checker output, verbatim:
    ...
    FAIL  public-sensitive-literals  (.claude/handoffs/2026-08-20-122706-session-title-as-address.md:5:- Project: ~/git/agent-scripts

Redacted to `~/git/agent-scripts`.

## Class 2 — Tailscale IPs (lines 33 / 87 / 173, surfaced only after class 1 was fixed)

    33:  to two hosts (`local` and `.62`), verified at SHA
    87:  - [x] Pushed to `origin/main`, deployed to `local` and `.62`, both verified.
    173: - Tailscale: this host is `.44`; the second deploy target is `.62`.)
