...
A bare quote with no task binding is an invalid receipt. Paraphrased, reworded, or irrelevant quotes = gate FAILED. File missing or unreadable = gate FAILED and the gated action must not be performed. The first matching gated action must not start before the receipt; output or actions past a failed gate are invalid — stop, disclose, redo where reversible. Routine receipts are audit evidence, not progress narration: do not emit them as user-facing commentary. Surface a receipt only for a policy deviation, approval boundary, BLOCK/escalation, or explicit user request — and a user-facing receipt is the COMPACT form: ONE line, `GATE: ~/.agents/rules/<file> §<n> — <plain-word conclusion>`, home paths abbreviated with `~`, NO verbatim rubric text (the full quote lives in the artifact); it obeys the output-shaping rules (lead with the outcome, one line, no filler). Reuse the existing receipt for follow-ups under the same role, rubric, and acceptance; otherwise update the artifact before acting.
...
- External-worker supervision: every authorized asynchronous external CLI worker (`codex-tmux`, Claude/AGY/GLM CLIs — native Agent work needs NO proxy) is owned by exactly ONE supervision-only native sub-agent proxy, never the parent; the proxy is event-driven (waits on the tool-layer blocking supervisor, never polls). After handoff the parent MUST NOT call `status`/`watch`/`capture`/`probe`/`ping`/`result` or send follow-ups unless the proxy reports blocked/lost-liveness, terminates unexpectedly, or the user asks. Supervision model/agent type: `model-dispatch.md §5` (read at the delegation gate). One-shot bounded calls exempt; without native sub-agents, run directly and report `UNAVAILABLE-NATIVE`.
...
## Continuity
- Non-trivial work (more than ~3 steps, spans 2+ files, needs background work, or is likely to be interrupted): manage it with `codex-dynamic-workflows` conventions (`.workflow/<YYYYMMDDHHMM>-<slug>/` with `plan.md`, `state.json`, `orchestration.md`; timestamp = run creation time, so `ls .workflow/` reads as a timeline; resume by globbing `*-<slug>`) — no ad-hoc task_plan.md. Before starting, judge the task shape: loop-shaped work (audit / consensus verification / findings triage / root-cause deep-dive / plan→build) enters the `using-workflows` router first to pick a recipe — Claude runs it natively via Workflow; Codex runs it via the `claude-workflow-runner` adapter in that skill's `references/codex-adapter.md` (runner = cheapest capable model, reasoning effort low). Recipe outputs land back in the SAME `.workflow/<slug>/` run directory. Non-loop-shaped work just creates the run directory and proceeds. Reference: https://github.com/scasella/claude-dynamic-workflows-codex
