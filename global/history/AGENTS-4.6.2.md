...
Version: 4.6.2-lean-routed
Provenance: derived from v4.6.1-lean-routed; marks the shared routed-rule home as DEPLOYED, notes its planned git home (agent-scripts), and removes stale NOVA canary references.
...
Canonical routed-file directory (DEPLOYED): `~/.agents/rules/`. Both Codex and Claude Code read routed files on demand from this canonical path. No rules-directory symlink or eager import is used. Edits follow `~/.agents/rules/maintenance.md`.
