...
| stress-test a plan/decision/idea (frontier rounds; add `domain-modeling` for ADR/glossary output) | `batch-grill-me` | SKILL · manual |
...
- **Grilling** → `batch-grill-me` (the ONLY grill member — asks each frontier of questions in one round; pair with `domain-modeling` when ADRs/glossary should be written as you go).
