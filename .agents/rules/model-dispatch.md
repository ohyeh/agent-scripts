...
Worker lifecycle (2026-08-18 user ruling): workers are SESSION TEAMMATES, not
disposables — they live and die with the session. Default = ONE persistent named
worker per CLI per session: bring it up once (first task via `assign`), feed every
later task to the SAME worker with `send-wait` (each hosted the same way — proxy),
`stop` only at session end. Per-task start/stop churn is a defect: it burns
bring-up cost and amplifies the upstream Codex FD-leak (lessons 2026-08-18 EMFILE).
One-shot throwaway workers are the EXCEPTION, only for isolation (different
repo/trust scope) or genuine parallel fanout.
