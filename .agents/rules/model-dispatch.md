...
Worker lifecycle (2026-08-18 user ruling): workers are SESSION TEAMMATES, not disposables — they live and die with the session. Team slot cap (user ruling 2026-08-18): at most 3 persistent named workers per session TOTAL across all CLIs — opening more requires the user's explicit request. Bring a worker up once (first task via `assign`), feed every later task to the SAME worker with `send-wait` (each hosted the same way — proxy), `stop` only at session end. Per-task start/stop churn is a defect: it burns bring-up cost and amplifies the upstream Codex FD-leak (lessons 2026-08-18 EMFILE). One-shot throwaway workers are the EXCEPTION, only for isolation (different repo/trust scope) or genuine parallel fanout. Every `assign` owes a matching `stop`: before ending a session or reporting done, list live workers and stop every one this session created — an orphan holds FDs. Observed shape (session ad1e2dec, a full supervised dev day, not a runaway): 48 `assign` vs 29 `stop` calls and 67 background agents still registered when the user cleared them; the assign/stop delta is the reap-discipline signal, the wave count is legitimate work. The 3-slot cap counts concurrent background subagents too, not just tmux workers: one supervision proxy per live worker is the whole budget, and any per-item reviewer fanout exceeding it needs the user's explicit request.
