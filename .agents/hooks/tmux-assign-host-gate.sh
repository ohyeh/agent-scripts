...
if [ "$HIT" = "assign" ]; then
  ...
  echo "BLOCKED: parent-session foreground \`agent-tmux <cli> status|capture|probe|result\` is per-worker polling — retired (W32) and superseded by the supervision proxy (model-dispatch.md §4, 2026-08-17 ruling). Wait for the proxy's terminal report instead. If a fallback harvest is genuinely needed, run it as a background task (run_in_background) with the reason logged in the run dir." >&2
fi
exit 2
