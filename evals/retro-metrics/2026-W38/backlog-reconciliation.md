...
| Y4 | hook matcher ＋ collector hook-recovered（G6/G7），P0 | matcher：`~/.agents/hooks/claim-evidence-gate.sh:56,59` 正則仍為 W37 版型（有 `VERDICT: *PASS`、無 backlog 文字排除）。collector：`agent-sessions.mjs` 無 `hook-recovered`（grep 0 hit）。 | claim-evidence blocked 7d：.44 48/110、.62 61/137（probe）。誤打回逐筆判別未做（Z3）。本 session 被打回 2 次，皆為「done 字眼無 token」型。 | 未做 |
...
W37 的下週 backlog 用 Y 序列（Y1–Y7＋2）；本週初稿沿用 X 序列寫成 X9–X16，與 W36 的 X1–X8 相接、跳過 Y。09-18 Paul 授權自取一致方案：全部改為 Z1–Z11（含 09-18 追加三條），見 next-week-backlog.md 開頭。
